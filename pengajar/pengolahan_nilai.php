...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // === [BARU] CEK STATUS KUNCI SEBELUM MENYIMPAN (KEAMANAN BACKEND) ===
    $stmt_cek_kunci = $koneksi->prepare("SELECT status FROM status_kunci_nilai WHERE id_kelas = ? AND semester = ? AND tahun_ajaran = ?");
    $stmt_cek_kunci->bind_param("iss", $id_kelas, $semester, $tahun_ajaran);
    $stmt_cek_kunci->execute();
    $res_cek_kunci = $stmt_cek_kunci->get_result()->fetch_assoc();
    $stmt_cek_kunci->close();

    if ($res_cek_kunci && $res_cek_kunci['status'] == 'terkunci') {
        // Jika terkunci, tolak simpan dan redirect dengan pesan error
    ...
    try {
        ...
        for ($i = 0; $i < count($santri_ids); $i++) {
            ...
            if (!empty($id_nilai)) {
                // Update jika ID nilai ada
                 $stmt_update->bind_param("ssi", $nilai, $catatan, $id_nilai);
                 $stmt_update->execute();
            } elseif ($nilai !== '') { 
                // Insert hanya jika nilai tidak kosong
...
// === [BARU] CEK STATUS KUNCI UNTUK TAMPILAN (FRONTEND) ===
$is_locked = false; // Default tidak terkunci
if ($selected_kelas_id && $selected_semester && $selected_tahun) {
    $stmt_lock = $koneksi->prepare("SELECT status FROM status_kunci_nilai WHERE id_kelas = ? AND semester = ? AND tahun_ajaran = ?");
    $stmt_lock->bind_param("iss", $selected_kelas_id, $selected_semester, $selected_tahun);
    $stmt_lock->execute();
    $res_lock = $stmt_lock->get_result();
    if ($row_lock = $res_lock->fetch_assoc()) {
        if ($row_lock['status'] == 'terkunci') {
            $is_locked = true;
        }
    }
    $stmt_lock->close();
...
<head>
    ...
    <style>
        /* === CSS TEMPLATE GABUNGAN (NON-RESPONSIVE / MOBILE STYLE) === */
        :root {
            --warna-hijau: #00a86b;
...
<body>
        ...
        <main class="dashboard-area">
            ...
            <?php if ($is_locked): ?>
                <div class="alert-locked">
                    ...
                    <div>
                        PERHATIAN: Pengisian nilai untuk kelas dan semester ini telah <strong>DIKUNCI</strong> oleh Admin. Anda hanya dapat melihat data.
                    </div>
                </div>
            <?php endif; ?>
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" name="nilai[]" class="input-nilai nilai-input" 
                                               value="<?php echo htmlspecialchars($siswa['nilai'] ?? ''); ?>" 
                                               min="0" max="100"
                                               <?php echo $is_locked ? 'disabled' : ''; ?>
                                        >
                                    </td>
                                    <td>
                                        <input type="text" name="catatan[]" class="input-catatan" 
    ...
    <script>
        ...
        // 3. JS Pop-up Sukses/Gagal/Terkunci
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (status === 'sukses') {
                ...
                Swal.fire({
                    title: 'Terkunci!',
                    text: 'Maaf, data nilai untuk kelas ini sudah dikunci oleh Admin.',
                    icon: 'warning',
